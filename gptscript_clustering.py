...
if __name__ == "__main__":
    ...
    # print(data_obj.dataset[41][0])
    # print(data_obj.dataset[41][1])

    embedder = SentenceTransformer('all-MiniLM-L6-v2')

    corpus = [d[0] for d in data_obj.dataset]
    cqs = [d[1] for d in data_obj.dataset]
    corpus_embeddings = embedder.encode(corpus, convert_to_tensor=True)

    result_dict = {}
    top_k = 3  # Define top_k here

    for query in data_obj.dataset:
        story = query[0].split('\n')[1]
        history_turns = query[0].split('\n')[3]
        ambiguous_question = query[0].split('\n')[5]

        query_text = f"Story:\n{story}\nHistory:\n{history_turns}\nAmbiguous Question:\n{ambiguous_question}"

        query_embedding = embedder.encode(query_text, convert_to_tensor=True)
        cos_scores = util.cos_sim(query_embedding, corpus_embeddings)[0]
        top_results = torch.topk(cos_scores, k=top_k + 1)  # Add 1 to exclude the key itself

        key = query_text + " Clarifying Question: "
        values = []

        for score, idx in zip(top_results[0], top_results[1]):
            if corpus[idx] != key:
                values.append((corpus[idx] + cqs[idx], score.item()))

        values = values[1:3]  # Keep only the 2nd and 3rd highest scores
    ...
    print("Results saved in result_dict.json")
